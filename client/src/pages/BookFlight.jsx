...
const BookFlight = () => {
    ...
    async function onToken(token)
    {
      console.log(token)
        ...
        const inputs ={user: localStorage.getItem('userId'), flight: id, flightName, flightId, departure: StartCity, journeyTime: startTime, destination: destinationCity, 
                                                    email, mobile, passengers: passengerDetails, totalPrice, 
                                                    journeyDate, seatClass: coachType,token} 
        
        await axios.post('http://localhost:6001/book-ticket', inputs).then(
          (response)=>{
            alert("booking successful"); navigate('/bookings');
          }
        ).catch((err)=>{
    ...
    return (
      <div className='BookFlightPage'>
        ...
        currency='INR'
        stripeKey="pk_test_51QMCy8Fd6w6hK8ymYHGwljf0fVekzKNeffPXg13soxh1fyfORcaCoWHTznuaxs20KWj4YM41sG8jqrZYHMbBtmQI00kFmao6EY"
      >
